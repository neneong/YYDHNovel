...
<%@page import="dao.NovelDAO"%>
	<%
		String tag = (String)request.getParameter("title");
		NovelDAO nd = new NovelDAO();
		NovelTagDAO ntd = new NovelTagDAO();
		TagDAO td = new TagDAO();

		ArrayList<String> tags = nd.getNovelListByTitle(tag);
	...
	<div class="mb-3">	
      	<form action="/search/keyword.jsp" class="input-group mb-3">
          <input type="text" name="title" class="form-control" placeholder="검색할 글자를 입력해주세요">
          <button type="submit" id="basic-addon2" class="input-group-text btn btn-secondary ">검색</button>
        </form>
    </div>
    
         <nav class="nav nav-pills">
          <a class="nav-link active" href="/search/keyword.jsp">제목</a>
