...
<%@page import="dao.NovelDAO"%>
	<%
		...
		ArrayList<String> writters = nd.getNovelListByWritter(writer);
		ArrayList<String> tagName = new ArrayList<String>();
		for (String tag2 : writters) {
	...
	<div class="mb-3">	
      	<form action="/search/writer.jsp" class="input-group mb-3">
          <input type="text" name='writer' class="form-control" placeholder="검색할 글자를 입력해주세요">
         ...
         <nav class="nav nav-pills">
          ...
          <a class="nav-link aria-current="page" href="/search/hashTag.jsp">태그</a>
          <a class="nav-link active" href="/search/writer.jsp">작가</a>
        </nav>
        
        
        <%
        	for(String ta:writters){
